...
void eliminate (vector<vector<float> >& A, vector<float>& pivotA, int n, int pid, int num_procs) { /*triangulize the matrix A*/
    ...
    int pid_n = (pid+1)%num_procs; // next pid - for sends
    int pid_p = (pid + (num_procs-1))%num_procs; // previous pid - for receives
    ...
    for (k=0; k<n; k++) { /*loop over all diagonal (pivot) elements*/
        ...
        if (pivot_pid == pid) {
            ...
            for (j=k+1; j<n; j++) {/*for all elements in pivot row, to the right of pivot*/
                ...
                if (num_procs>1)
                    MPI_Send(&pivotA[j], 1, MPI_FLOAT, pid_n, 2, MPI_COMM_WORLD);
            }
        ...
        // receive pivotA if applicable
            ...
            for (j=k+1; j<n; j++) {
                MPI_Recv(&pivotA[j], 1, MPI_FLOAT, pid_p, 2, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                // send received pivotRow to next pid - if not pivot_pid (to avoid corner case from circular loop)
                if (pid_n != pivot_pid)
                    MPI_Send(&pivotA[j], 1, MPI_FLOAT, pid_n, 2, MPI_COMM_WORLD);
            }
